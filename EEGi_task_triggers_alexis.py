...
#Parameters
#size
radius= 7.5 # he tenido que reducir el radio de 8 a 7.5 para que cupiese en la pantalla (Alexis)
size_stim= 1 
#time
...
screen = [3200,1800] ## pixels (screen resoulution)
#screen = [1280, 800] ## pixels (screen resoulution)
width, length = screen[0], screen[1]
#inches of the screen diagonal (check o n All settings --> Displays or internet: http://howbigismyscreen.co/ )
#Has de vigilar segun si es full screen o no... siempre será la diagonal de la screen que aparezca!!!
diagonal= 13  #### in inches!!!!! ## screen psycho 22.05 (47.4cm x 29,8cm --> 56 cm de diagonal--> 56cm * 0.398 inches/cm = 22.05 inches)
...
def save_output(output, filename):
    ...
    pathname =  os.path.join(root , 'results',filename) #root + '\\results\\' + filename #decide the path to create the xlsx
...
if sst: p_port.write(b'10') #start instructions
if sst: p_port.write(b'00')
...
if sst: p_port.write(b'11') #start trials
if sst: p_port.write(b'00')
...
for i in range(0,len(stimList)):    
    ...
    for frameN in range(frames_cue_present):
        if frameN == 0: 
             if sst: win.callOnFlip(p_port.write, b'01') ## 1 is cue
        if frameN == 1:
    ...
    if order==1:
        ...
        for frameN in range(frames_stim_present):
            if frameN == 0: 
                if sst: win.callOnFlip(p_port.write, b'02')  #2 target
            if frameN == 1:
                if sst: win.callOnFlip(p_port.write, b'00') 
        ...
        for frameN in range(frames_stim_present):
            if frameN == 0: 
                if sst: win.callOnFlip(p_port.write, b'03')  #3 distractor
            if frameN == 1:
                if sst: win.callOnFlip(p_port.write, b'00')
    ...
    for frameN in range(frames_delay1):
        if frameN == 0: 
            if sst: win.callOnFlip(p_port.write, b'04')   # 4 start delay1
        if frameN == 1:
            if sst: win.callOnFlip(p_port.write, b'00')
        fixation(); 
    ...
    if sst: p_port.write(b'05') # 5 end delay1
    if sst: p_port.write(b'00')
    ...
    for frameN in range(frames_delay2):
        if frameN == 0: 
             if sst: win.callOnFlip(p_port.write, b'06') #start delay2
        if frameN == 1:
            if sst: win.callOnFlip(p_port.write, b'00') 
        fixation(); 
    ...
    if sst: p_port.write(b'07') #end delay2
    if sst: p_port.write(b'00')
    ...
    if sst: p_port.write(b'08') #start response
    if sst: p_port.write(b'00')
    ...
    #Angle response 
    if sst: p_port.write(b'09') #response time
    if sst: p_port.write(b'00')
...
Final_text.draw()
win.flip()
if sst: p_port.write(b'12') #end experiment
if sst: p_port.write(b'00')
